...
/**
 * Spring @Configuration which does not require a running database.
 * It also does not load any job configuration (as they are in the DB), thus nor
 * starts any background jobs. For some integration tests, this may be
 * perfectly sufficient (and faster to run such tests).
 */
@Configuration
public class TestsWithoutDatabaseAndNoJobsConfiguration extends TestSpringConfiguration {
    ...
    // Another @Bean that would be sensible to mock in this @Configuration (with a separate test) would be:
